...
class Customer {
    static async getAllCustomers(page = 1, limit = 10, search = '') {
        ...
        if (search) {
            query += ' AND (full_name LIKE ? OR customer_id LIKE ? OR contact_number LIKE ?)';
    ...
    static async getCustomerByRFID(rfidNumber) {
        const [rows] = await db.query('SELECT * FROM customers WHERE rfid_number = ?', [rfidNumber]);
        return rows[0];
    }
